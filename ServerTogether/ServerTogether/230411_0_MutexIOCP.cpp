...
{
	//락이랑 언락사이가 개길어서 성능이 걱정된다
	// 근데 디스콘넥트는 자주 안하니까 성능 너무 큰일은 없을거임
	...
	//	clients[c_id]._st = ST_FREE;
	// 여기에서 이걸 하는 경우도 있지만 위험함 >> 재사용의 문제
	for (auto& pl : clients) {
		if (pl._st != ST_INGAME) continue;
		if (pl._id == c_id) continue;
	...
	clients[c_id]._st = ST_FREE;
	/************************************************************
	 * Critical Section 구간이 너무 길다.
	 * 루프 앞으로 옮겨야 하는데 그러면 session
	 * 재사용으로 오작동할 수 있다.
	 *
	 * 해결법은 Shadered Ptr 사용하기
	*************************************************************/
	clients[c_id].cl.unlock();
}
