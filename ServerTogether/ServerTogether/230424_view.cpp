...
{
    //return VIEW_RANGE* VIEW_RANGE >= ((clients[a].x - clients[b].x) ^ 2 + (clients[a].y - clients[b].y) ^ 2);// sqrt와 ^ 는비용이 커진다.  sqrt할바에 그냥 곱하기 할래
                                                                             //2D와 3D의 시야 개념이 다르다. 거리로 시야를 판단하는데 2D는 모니터 화면이 시야이다.
                                                                             // 
...
void process_packet(int c_id, char* packet)
{
    ...
    case CS_LOGIN: {
        ...
        for (auto& pl : clients) {
            ...
            //if (can_see(pl._id, c_id) != false)continue;
