...
class SESSION {
private:
	size_t _id;	// int를 size_t로 교체 >> 64비트 환경에 맞춤
	WSABUF _recv_wsabuf;
	WSABUF _send_wsabuf;
...
public:
	...
	void do_recv() {
		...
		_recv_over.hEvent = reinterpret_cast<HANDLE>(_id);	// 32비트에서는 Handle도 32비트, 64비트에서 Handle 64비트
...
int main()
{
	...
	for (int i = 1; ; ++i) {
		...
		clients.try_emplace(i, i, c_socket);
			// emplace 하는 이유 : 이중 생성 (그냥 assign하면 생성자 또 불림)
		clients[i].do_recv();
			// 원래는 생성자에서 해주는게
	}
	clients.clear();
