...
public:
	...
	SESSION(int id, SOCKET s) : _id(id), _socket(s), x(0), y(0), _prev_remain(0)
	{
		_name[0] = 0;
	}
	...
	void do_recv()
	{
		...
		_recv_over._wsabuf.len = BUF_SIZE - _prev_remain;				//패킷 재조립을 해야기 때문에 이전에 받아놓은 부분부터 이어서 받아야한다. 
		_recv_over._wsabuf.buf = _recv_over._send_buf + _prev_remain;	//그래서 prev_remain만큼 받는 바이트 수가 줄어든다.
