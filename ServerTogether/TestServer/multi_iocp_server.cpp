...
public:
    ...
    mutex _vl;   // 위에 놈 data race 잡아주기 위해서
...
void process_packet(int c_id, char* packet)
{
    ...
    case CS_MOVE: {
        ...
        // 시야에 있는 애들한테만 이동을 알려주자

        unordered_set<int> new_vl; // 새로운 view list
        for (auto& pl : clients)
        {
            if (pl._id == c_id) continue;   // 나 자신은 건너뛰기
        ...
        for (auto& npl : new_vl)
        {
            ...
            // new view list에 있는데 기존 view list에 없다면
        ...
        // 여기서 lock 해버리면 lock과 unlock 사이가 너무 길어짐
        //clients[c_id]._vl.unlock();
    }
    }
}
